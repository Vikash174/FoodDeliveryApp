...
const BestOffers = (props) => {
  ...
  const bestOfferBanners =
    bestOffers[0]?.card?.card?.gridElements?.infoWithStyle?.info;

  return (
    <div className="border-b-2 border-b-gray-100">
      <div className="flex justify-between">
        <Title title={'Best offers for you'} />
        <NextPrevBtn element={bannerContainer} />
      </div>
      <div
        id="banner-container"
...
const Image = (props) => {
  const { imageId, accessibility } = props?.banner;
  return (
...
export const NextPrevBtn = (props) => {
  ...
  return (
    <div className="hidden md:flex">
      ...
      <button
        className="p-2 m-2 bg-gray-300 rounded-3xl"
